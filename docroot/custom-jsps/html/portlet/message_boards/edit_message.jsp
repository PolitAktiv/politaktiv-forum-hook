...
<liferay-ui:header
	...
	title='<%= (curParentMessage != null) ? LanguageUtil.format(pageContext, "reply-to-x_pa-custom", curParentMessage.getSubject()) : (message == null) ? "add-message_pa-custom" : LanguageUtil.format(pageContext, "edit-x_pa-custom", message.getSubject()) %>'
/>
...
<aui:form action="<%= editMessageURL %>" enctype="multipart/form-data" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveMessage(false);" %>'>
	...
	<aui:fieldset>
		...
		<c:if test="<%= curParentMessage == null %>">
			...
			<aui:input disabled="<%= disabled %>" helpMessage="message-boards-message-question-help_pa-custom" label="mark-as-a-question" name="question" type="checkbox" value="<%= question %>" />
		</c:if>

		<c:if test="<%= (message == null) && themeDisplay.isSignedIn() && allowAnonymousPosting %>">
			<aui:input helpMessage="message-boards-message-anonymous-help_pa-custom" name="anonymous" type="checkbox" />
		</c:if>
		...
		<c:if test="<%= (message == null) && themeDisplay.isSignedIn() && !SubscriptionLocalServiceUtil.isSubscribed(themeDisplay.getCompanyId(), user.getUserId(), MBThread.class.getName(), threadId) && !SubscriptionLocalServiceUtil.isSubscribed(themeDisplay.getCompanyId(), user.getUserId(), MBCategory.class.getName(), categoryId) %>">
			<aui:input helpMessage="message-boards-message-subscribe-me-help_pa-custom" label="subscribe-me" name="subscribe" type='<%= (MBUtil.getEmailMessageAddedEnabled(portletPreferences) || MBUtil.getEmailMessageUpdatedEnabled(portletPreferences)) ? "checkbox" : "hidden" %>' value="<%= subscribeByDefault %>" />
		</c:if>
		...
		<c:if test="<%= message == null %>">
			<c:choose>
				<c:when test="<%= bShowEditMsgPermSection %>">
				...
				<c:otherwise>
					<div style="position:absolute;z-index:0;visibility:hidden;">
						<aui:field-wrapper label="permissions">
							<liferay-ui:input-permissions
								modelName="<%= MBMessage.class.getName() %>"
							/>
